...
local function predicateSawTree(item)
...
function JB_Big_Wood.sawDownTree(playerObj, tree)
    ...
    walkTo:setOnComplete(function()
        ...
        if not saw or not predicateSawTree(saw) then
            local saw = playerObj:getInventory():getFirstEvalRecurse(JB_Big_Wood.utils.predicateTreeSaw)
